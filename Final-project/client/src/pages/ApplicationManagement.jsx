...
const ApplicationManagement = () => {
  ...
  useEffect(() => {
    const fetchApplications = async () => {
      try {
        ...
        const res = await axios.get("http://localhost:5500/api/admin/applications", {
          headers: {
  ...
  const handleDeleteApplication = async (id) => {
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://localhost:5500/api/admin/applications/${id}`, {
  ...
  const handleUpdateStatus = async (id, status) => {
    try {
      const token = localStorage.getItem("token");
      const res = await axios.patch(`http://localhost:5500/api/applications/${id}/status`, { status }, {
        headers: { Authorization: `Bearer ${token}` }
      });
